<div class="card">
    <div class="card-header text-center">
        <h2 class="card-title status-title m-0">
            สถานะการสมัครประกวดรางวัลอุตสาหกรรมท่องเที่ยวไทย ครั้งที่ 14 ประจำปี 2566
        </h2>
    </div>
    <div class="card-body">
        <div class="bs-row">
            <div class="col-4 text-center">
                <span class="fsm-20 active">1. กรอกแบบฟอร์มใบสมัคร</span><br>
                <span style="color: #F64E60;">ภายในวันที่ 28 กุมภาพันธ์ 2566</span>
            </div>
            <div class="col-4 text-center">
                <span class="fsm-20">2. กรอกข้อมูลแบบประเมินขั้นต้น (Pre-screen)</span>
            </div>
            <div class="col-4 text-center">
                <span class="fsm-20">3. สรุปผลการประเมิน</span>
            </div>
        </div>
        <div class="bs-row mt-3 justify-content-center">
            <div class="col-9">
                <div class="position-relative m-4">
                    <div class="progress" style="height: 4px;">
                        <div class="progress-bar bg-main" role="progressbar" aria-label="Progress" 
                        style="width: 0%;" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100"></div>
                    </div>
                    
                    <button type="button" style="width: 2rem; height:2rem;"
                    class="position-absolute top-0 start-0 translate-middle btn btn-sm btn-main rounded-pill">
                        <i class="bi bi-pencil-square"></i>
                        <i class="bi bi-check-lg hide"></i>
                    </button>
                    <button type="button" style="width: 2rem; height:2rem;"
                    class="position-absolute top-0 start-50 translate-middle btn btn-sm btn-secondary rounded-pill">
                        <i class="bi bi-three-dots"></i>    
                        <i class="bi bi-check-lg hide"></i>
                    </button>
                    <button type="button" style="width: 2rem; height:2rem;"
                    class="position-absolute top-0 start-100 translate-middle btn btn-sm btn-secondary rounded-pill">
                        <i class="bi bi-three-dots"></i>
                        <i class="bi bi-check-lg hide"></i>
                    </button>
                </div>
            </div>
        </div>
    </div>
</div>